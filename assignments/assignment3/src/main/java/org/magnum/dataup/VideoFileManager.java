...
public class VideoFileManager {
	...
	private Path getVideoPath(Video v){
		...
		return targetDir_.resolve("video"+v.getId()+".mp4");//resolve the path
	}
	...
	public void copyVideoData(Video v, OutputStream out) throws IOException {
		...
		Files.copy(source, out);//write tor the output stream
	}
	...
	public void saveVideoData(Video v, InputStream videoData) throws IOException{
		...
		Files.copy(videoData, target, StandardCopyOption.REPLACE_EXISTING);//save the data to output stream from the input stream
	}
