...
public class VideoServiceController {
    ...
    public @ResponseBody double setAndGetRatingForVideo(@PathVariable(VideoSvcApi.ID_PARAMETER) long id,@RequestParam(VideoSvcApi.ID_RATING) double rating, HttpServletResponse response) throws IOException {
        if (getDataStatus(id) == HttpServletResponse.SC_OK) { //if you can assert that the video metadata exist then we can retrieve it
            Video video =  videos.findOne(id);//get the video
