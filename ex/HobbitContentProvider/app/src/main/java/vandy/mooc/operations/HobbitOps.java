...
public class HobbitOps implements ConfigurableOps {
    ...
    /**
     * innitialize the access means to content resolver
     */
     ...
     */
    public HobbitOps() {
        accessMeansImplTrigger();//pick the default implementation of the implementation for the access means

    }

    private void accessMeansImplTrigger() {
        // Select the appropriate means of accessing the Content
        // Provider.
        switch(mAccessMeans) {
        case CONTENT_RESOLVER:
            mHobbitOpsImpl =
                new HobbitOpsContentResolver();//at the first time, the default will be the content resolver
            break;
     ...
     * it uses factory method pattern under the hood
     */
    ...
    /**
     * Display all the current contents of the HobbitContentProvider.
     */
    ...
    public void setContentProviderAccessMeans(ContentProviderAccessMeans accessMeans) {
        if(this.mAccessMeans!=accessMeans){
            mAccessMeans = accessMeans;
            accessMeansImplTrigger();//trigger to change the implementation fo the content provider based on the access Means

        }
        System.out.println("name of hobit ops impl"+mHobbitOpsImpl.getClass().getSimpleName());
